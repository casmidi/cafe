...
export default function ForgotPassword() {
    ...
    const [otp, setOtp] = useState(''); // Ganti nama state jadi OTP
    ...
    const handleRequest = async (e) => {
        ...
        setIsLoading(true);
        try {
            const res = await axios.post(API_URL + 'auth/customer-forgot', { email });
            if (res.data.status) {
                // Tampilkan pesan simulasi OTP agar user tahu
            ...
            showAlert('error', 'Error', 'Gagal mengirim request.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleReset = async (e) => {
        ...
        setIsLoading(true);
        try {
            // Kirim OTP sebagai token
            const res = await axios.post(API_URL + 'auth/customer-reset', { token: otp, new_password: newPassword });
            if (res.data.status) {
            ...
            showAlert('error', 'Error', 'Gagal reset password.');
        } finally {
            setIsLoading(false);
        }
    };
        ...
        <div className="min-h-screen flex items-center justify-center bg-brand-bg p-4 font-sans">
            <div className="bg-white w-full max-w-sm p-8 rounded-3xl shadow-2xl text-center border border-white">
                <div className="flex justify-start mb-4">
                    <Link to="/member/login" className="p-2 bg-gray-50 rounded-full text-gray-500 hover:bg-gray-100 transition-colors"><ArrowLeft size={20}/></Link>
                </div>
                
                <h2 className="text-2xl font-bold text-brand-darkest mb-2">Reset Password</h2>
                ...
                {step === 1 ? (
                    ...
                    <form onSubmit={handleReset} className="space-y-4">
                        <div className="bg-blue-50 p-3 rounded-xl text-xs text-blue-700 mb-2 text-left border border-blue-100">
                            Kode OTP telah dikirim ke <b>{email}</b>. Silakan cek kotak masuk atau folder spam Anda.
                        </div>
