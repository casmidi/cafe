...
import { Plus, Save, Move, Users, Trash2, X, Armchair, QrCode, Download, LayoutGrid, Map, CheckCircle } from 'lucide-react';
...
export default function TableLayout() {
    ...
    const [viewMode, setViewMode] = useState('map'); // 'map' atau 'grid'
    ...
    const handleDragEnd = async (e, tableId) => {
        ...
        let x = e.clientX - containerRect.left - 48; // Center cursor
        let y = e.clientY - containerRect.top - 48;
        ...
        // Simpan Background
        try {
            await axios.post(API_URL + 'master/tables/position', {
                id: tableId, x, y
            }, { headers: { Authorization: `Bearer ${token}` } });
        } catch (error) {
            console.error("Gagal simpan posisi", error);
        }
    };
    ...
    const handleAddTable = async (e) => {
        ...
        try {
            // Default posisi di tengah area visible
            await axios.post(API_URL + 'master/tables', { 
                ...newTable, position_x: 50, position_y: 50 
            }, { headers: { Authorization: `Bearer ${token}` } });
        ...
        <div className="space-y-6 h-[calc(100vh-120px)] flex flex-col">
            ...
            <div className="flex-1 bg-gray-100 rounded-2xl border-2 border-dashed border-gray-300 relative overflow-hidden shadow-inner group">
                ...
                {viewMode === 'map' ? (
                    ...
                    <div className="w-full h-full relative overflow-auto" ref={containerRef}>
                        {tables.length === 0 && (
                            <div className="absolute inset-0 flex flex-col items-center justify-center text-gray-400 pointer-events-none">
                                <Armchair size={64} className="mb-4 opacity-20" />
                        ...
                        {tables.map((table) => (
                            <motion.div
                                ...
                                initial={{ x: parseInt(table.position_x), y: parseInt(table.position_y) }}
                                animate={{ x: parseInt(table.position_x), y: parseInt(table.position_y) }}
                    ...
                    // --- MODE LIST (GRID) ---
                    <div className="p-6 grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 overflow-y-auto h-full content-start">
                        {tables.map((table) => (
                            <div key={table.id} className={`bg-white p-4 rounded-2xl border shadow-sm hover:shadow-md transition-all flex flex-col items-center justify-center gap-2 relative group ${getStatusColor(table.status)}`}>
                                <div className="w-12 h-12 bg-gray-50 rounded-full flex items-center justify-center text-brand-darkest relative">
                                    <Armchair size={24}/>
                                    {table.status === 'occupied' && (
                                ...
                                <div className="text-center">
                                    <h4 className="font-bold text-lg">Meja {table.table_number}</h4>
